...
(defn left-pad
  [s padding n]
  (if (< (count s) n) (recur (str padding s) padding n) s))
...
(defn output-file
  ...
  (let [sub-dirs (->> (fs/components (:input-file source))
        ...
        year-month-day (when (-> source
                                 :metadata
                                 :local-date)
                         [(str (.getYear (-> source
                                             :metadata
                                             :local-date)))
                          (left-pad (str (.getMonthValue (-> source
                                                             :metadata
                                                             :local-date)))
                                    "0"
                                    2)
                          (left-pad (str (.getDayOfMonth (-> source
                                                             :metadata
                                                             :local-date)))
                                    "0"
                                    2)])]
...
(defn assert-output-files!
  ...
  (let
    [r
     [(= "404.html"
         (str (output-file
               {:metadata {:layout "page" :title "Not found" :as-html-page true}
                :html "<p>:(</p>"
        ...
        (output-file
         {:metadata
          {:layout "page"
           ...
           :description
           "A long weekend with Jenn climbing and camping in the Red River Gorge"}
          :input-file
          (fs/file
           "posts/adventures/2022-10-14-2022-10-18-a-beautiful-fall-trip-to-the-red-river-gorge.markdown")})))
      (= "blog/reading-in-2024/index.html"
         (str (output-file
               {:input-file (fs/file
                             "posts/blog/2024-01-05-reading-in-2024.markdown")
                :metadata {:date "2025-01-05 16:23 -0600"
                           :local-date (LocalDate/of 2025 1 5)}})))]]
    (when (some false? r)
      (with-out-str (println "Problem with output-file paths, results:" r)))))
...
(defn parse-datetime
  [s]
  (let [parse (fn [f]
                (try (LocalDateTime/parse s f)
                     (catch DateTimeParseException _
                       (try (LocalDate/parse s f)
...
(defn ->yyyy-MM-dd [d] (.format d (DateTimeFormatter/ofPattern "yyyy-MM-dd")))
...
(defn- rfc-3339
  [local-date]
  (let [fmt (DateTimeFormatter/ofPattern "yyyy-MM-dd'T'HH:mm:ssxxx")
        now (ZonedDateTime/of (.atTime local-date 23 59 59)
                              java.time.ZoneOffset/UTC)]
    (.format now fmt)))

(defn date->human-readable
  [d]
  (.format d
           (.withLocale (DateTimeFormatter/ofLocalizedDate FormatStyle/LONG)
                        Locale/ENGLISH)))
  ...
  (let [date
        (when (-> source
          ...
          (let [d (-> source
                      :metadata
                      :date)
                parsed (cond (inst? d) (LocalDateTime/ofInstant (.toInstant d)
                                                                (ZoneId/of
                                                                 "UTC"))
                             :else (parse-datetime d))]
            (if (instance? LocalDate parsed) parsed (.toLocalDate parsed))))
        source
        (cond-> source
          (-> source
              :metadata
              :start-date)
          (update-in [:metadata :start-date]
                     #(.toLocalDate (.atZone (.toInstant %) (ZoneId/of "UTC"))))
          (-> source
              :metadata
              :end-date)
          (update-in [:metadata :end-date]
                     #(.toLocalDate (.atZone (.toInstant %) (ZoneId/of "UTC"))))
          (-> source
              :metadata
              :updated)
          (update-in [:metadata :updated]
                     #(.toLocalDate (.atZone (.toInstant %)
                                             (ZoneId/of "UTC")))))]
    (cond-> source
      date (-> (assoc-in [:metadata :local-date] date)
               (assoc-in [:metadata :published-yyyymmdd] (->yyyy-MM-dd date))
               (assoc-in [:metadata :published-readable]
                         (date->human-readable date))
               (assoc-in [:metadata :published-rfc3339] (rfc-3339 date)))
      (-> source
          :metadata
          :start-date)
      (assoc-in [:metadata :start-date-readable]
       (date->human-readable (-> source
                                 :metadata
                                 :start-date)))
      (-> source
          :metadata
          :end-date)
      (assoc-in [:metadata :end-date-readable]
       (date->human-readable (-> source
                                 :metadata
                                 :end-date)))
      (-> source
...
(defn assert-blog-url!
  ...
  (let
    [r
     [(= "/hello/" (blog-url "hello"))
      (= "https://jakemccrary.com/hello/" (blog-url blog-root "hello"))
      (= "https://jakemccrary.com/hello/" (blog-url blog-root "/hello"))
      (= "https://jakemccrary.com/hello/" (blog-url blog-root "/hello/"))
      (= "https://jakemccrary.com/about.html"
         (blog-url blog-root "/about.html"))
      (=
       "https://jakemccrary.com/adventures/deep-water-soloing-over-kinkaid-lake/"
       (blog-url
        blog-root
        "adventures/deep-water-soloing-over-kinkaid-lake/index.html"))]]
    (when (some false? r)
      (with-out-str (println "Problem with blog-url, results:" r)))))
  ...
  (when (fs/exists? source-dir)
    (for [path (fs/glob source-dir "**.markdown")
          :let [source (as-> (markdown->source path) source
                         ...
                         (assoc-in source
                          [:metadata :canonical]
                          (blog-url blog-root (:output-file source))))]
...
(defn blog-articles
  [sources]
  (sort-by (comp :local-date :metadata) (filter blog-article? sources)))

(defn- categories
  [sources]
  (disj (into #{} (comp (map :metadata) (mapcat :categories)) sources) nil))

(defn write-html!
  [output-file m]
  (fs/create-dirs (fs/parent output-file))
  (when (fs/exists? output-file)
    (throw (Exception. (str "Tried to write file but it already exists:"
                            output-file))))
  (spit output-file
        (selmer.parser/render (or (:template m)
...
(defn- article-list
  ...
  [:ul {:class "post-list"}
   (for [article articles
     ...
     [:li
      [:div.post-meta [:time (str (:local-date metadata))]
...
(defn write-index!
  ...
  (write-html! (fs/file output-dir "index.html")
               {:body (hiccup/html [:div
                                    (article-list (reverse (blog-articles
                                                            sources)))])}))
...
(defn write-archive!
  ...
  (let [articles (reverse (blog-articles sources))]
    (write-html! (fs/file output-dir "blog" "archives" "index.html")
                 {:body (hiccup/html [:div {:id "blog-archives"}
   ...
   (->
     (xml/sexp-as-element
      [::atom/feed {:xmlns "http://www.w3.org/2005/Atom"}
       [::atom/title
        [:-cdata
         (str "Jake McCrary's articles" (when category (str " on " category)))]]
       [::atom/link {:href "https://jakemccrary.com/atom.xml" :rel "self"}]
       [::atom/link {:href blog-root}] [::atom/updated (rfc-3339-now)]
       [::atom/id blog-root]
       [::atom/author [::atom/name [:-cdata "Jake McCrary"]]]
       (for [article articles
    ...
    (xml/sexp-as-element
     [:urlset
      ...
      (concat (for [source sources
                    :when (not (clojure.string/blank? (:html source)))]
                [:url [:loc (str blog-root (str (:output-file source)))]
                 (when (-> source
                           :metadata
                           :local-date)
                   [:lastmod
              ...
              [[:url [:loc blog-root] [:lastmod (rfc-3339-now)]]])])
    xml/indent-str))
         ...
         (cske/transform-keys
          ...
          {:version "https://jsonfeed.org/version/1.1"
           ...
           :items (vec (for [article (take 20 (reverse (blog-articles sources)))
                             :let [output-file (:output-file article)
                                   {:keys [title local-date published]}
                                   (:metadata article)]
                             :when published
                             :let [link (str blog-root (str output-file))]]
                         (cond-> {:id link
             ...
             [:div.categories-page
              [:ul.category-list
               (for [category (sort (keys category->count))
                     :when category]
                 [:li
                  [:a {:href (str "/blog/categories/" category "/")} category
                   [:span.count
                    (str " (" (get category->count category 0) ")")]]])]])
  ...
  [:ul {:class "post-list"}
   (for [article articles
     ...
     [:li
      [:div.post-meta
       [:h2
        (if (clojure.string/blank? (:html article))
          (:title metadata)
        ...
        (clojure.string/join " "
                             (cons (->yyyy-MM-dd (:start-date metadata))
                                   (when (:end-date metadata)
                                     ["to"
                                      (->yyyy-MM-dd (:end-date metadata))])))]]
      (when-not (clojure.string/blank? (:description metadata))
        [:p.post-description (:description metadata)])])])

(defn- write-adventures!
  [sources]
  (let [adventures (->> sources
    ...
    (write-html! (fs/file output-dir "adventures" "index.html")
                 {:body (hiccup/html (adventure-list adventures))})))

#_{:clj-kondo/ignore [:clojure-lsp/unused-public-var]}
(defn render
  [{:keys [preview] :as args}]
...
(defn assert-code-highlighting!
  [sources]
  (let [supported-languages (set ["bash" "c" "clojure" "clojure-repl" "console" ;; not
                                                                                ;; really
                                                                                ;; there
                                  "org" ;; not available :shrug:
                                  "cpp" "html" ;; not there but works with
                                               ;; xml I guess?
                                  "java" "javascript" "lisp" "markdown" "pgsql"
                                  "ruby" "shell" "sql" "typscript" "xml"])
        required-languages (into #{} (mapcat extract-languages) sources)]
    (when-let [missing (seq (clojure.set/difference required-languages
      ...
      (with-out-str
        (println "Missing"
                 ...
                 "from highlight.js")
        (println
         "Go regenerate at https://highlightjs.org/download with languages:"
...
(defn test
  ...
  (let [errors (remove nil?
                       [(assert-code-highlighting! (load-sources true))
                        (assert-output-files!) (assert-blog-url!)])]
    (when (seq errors) (run! println errors) (System/exit -1))))
