...
(defmacro record-aggregate
  ...
  `(do
     ...
     (let [start-time# (System/nanoTime)
       ...
       (swap! aggregates update-in [~label :msecs] (fnil + 0) (/ (double (- end-time# start-time#)) 1000000.0))
       result#)))
...
I used the code similar to below. We looked at the code path we needed to optimize and wrapped chunks of it in record-aggregate. At the end of the calculations, we inserted a call to log-times so timing data would show up in our logs.
